...
void Arbiter::process_behaviors() {
    if (!this->behavior_queue.empty()) {
        wasd::behavior priority_msg = this->behavior_queue.top().second;
...
void Arbiter::cb_bh_wander(const wasd::behavior::ConstPtr &msg) {
    if (msg->active) {
        this->behavior_queue.push(std::pair<int, wasd::behavior>(PRIORITY_WANDER, *msg));
    }
    ROS_DEBUG("Arbiter: Wander(%s) Fw: %.1f Turn: %.1f", msg->active ? "on" : "off", msg->vel_fw, msg->vel_turn);
}

void Arbiter::move_robot(wasd::behavior& msg) {
